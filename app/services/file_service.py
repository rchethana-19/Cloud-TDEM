...
class FileService:
    ...
    async def ingest_file(
        ...
        try:
            ...
            if not await self.object_store.store(file_id, file_data, encrypted_package):
    ...
    async def retrieve_file(
        ...
        try:
            ...
            encrypted_data = await self.object_store.retrieve(file_id)
            
            if not encrypted_data:
                logger.error(f"Encrypted object not found: {file_id}")
                return None
            
            # Decrypt using Crypto Engine
