...
try:
    ...
    from crypto_engine.file_service import encrypt_file, decrypt_file
    from crypto_engine.integrity import calculate_hash, verify_integrity as verify_hash
...
except ImportError as exc:  # pragma: no cover - import failure should surface loudly
    logger.exception("Crypto Engine import failed")
    raise RuntimeError("Crypto Engine import failed from project root") from exc
...
class CryptoAdapter:
    ...
    async def verify_integrity(self, data: bytes, expected_hash: str) -> bool:
        ...
        try:
            return verify_hash(data, expected_hash)
        except Exception as e:
